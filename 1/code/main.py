...
def check(array,window_size=50): # check bouder
    results = array.copy()
    l = len(array)
...
def align(image1,image2,method='ed',levels=5):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str,required=True, help='the path of input image')
    parser.add_argument('--method', type=str,required=True, help='method of alignment: ed, ncc, sobel')
    args = parser.parse_args()
    # name of the input file
    imname = args.path
    img_name = imname.split('/')[-1].split('.')[0]
    # read in the image
    ...
    ag = align(g, b,method=args.method)
    ar = align(r, b,method=args.method)
